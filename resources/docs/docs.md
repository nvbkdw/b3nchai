...
The `home-routes` handler in the `bench-ai.routes.home` namespace
...
The `home-page` function will in turn call the `bench-ai.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `bench-ai.handler` namespace:
...
Request middleware functions are located under the `bench-ai.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `bench-ai.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `bench-ai.db.core` in some other namespace.
